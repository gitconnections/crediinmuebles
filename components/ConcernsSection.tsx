...
const iconMap = {
  "Proyectos seguros y confiables": ShieldOff,
  "Transparencia y plazos": FileWarning,
  "Calidad y valor a largo plazo": TrendingDown,
};
      ...
      <div className="container mx-auto px-6 lg:px-12">
        ...
        <Reveal delay={0.2}>
          <p className="text-lg text-center max-w-3xl mx-auto leading-relaxed text-foreground/80 mb-16">
            {content.concerns.description}
          </p>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {content.concerns.items.map((item, index) => {
            const IconComponent = iconMap[item.title as keyof typeof iconMap] || ShieldOff;
            return (
              <Reveal key={item.title} delay={0.3 + index * 0.1}>
                <div className="bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 flex flex-col items-center text-center">
                  <div className="bg-accent/10 text-accent p-4 rounded-full mb-6">
                    <IconComponent size={36} strokeWidth={2} />
                  </div>
                  <h3 className="text-2xl font-semibold font-poppins mb-4 text-primary">
                    {item.title}
                  </h3>
